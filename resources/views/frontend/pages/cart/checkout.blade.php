                                            ...
                                            <fieldset>
                                                ...
                                                @if (auth()->guard('customer')->check())
                                                    <input type="text" name="customer_first_name"
                                                        id="customer_first_name" class="form-control"
                                                ...
                                                @endif
                                                <p class="smaller text-danger">{{ $errors->first('customer_first_name') }}
                                                </p>
                                            </fieldset>
                                        </div>
                                        <div class="col-lg-6 col-md-12 col-12">
                                            <fieldset>
                                                ...
                                                @else
                                                    <input type="text" name="customer_last_name" id="customer_last_name"
                                                        ...
                                                        required />
                                                @endif
                                                <p class="smaller text-danger">{{ $errors->first('customer_last_name') }}
                                                </p>
                                            </fieldset>
                                        </div>
                                        <div class="col-lg-6 col-md-12 col-12">
                                            ...
                                            <fieldset>
                                                ...
                                                @if (auth()->guard('customer')->check())
                                                    <input type="text" name="customer_phone" id="customer_phone"
                                                        class="form-control"
                                                        value="{{ auth()->guard('customer')->user()->phone }}" required />
                                                @else
                                                    <input type="text" name="customer_phone" id="customer_phone"
                                            ...
                                            <fieldset>
                                                ...
                                                @if (auth()->guard('customer')->check())
                                                    <input type="text" name="customer_postal_code"
                                                        id="customer_postal_code" class="form-control"
                                                        value="{{ auth()->guard('customer')->user()->postal_code }}"
                                                        required />
                                                @else
                                                    <input type="text" name="customer_postal_code"
                                                        id="customer_postal_code" class="form-control"
                                                ...
                                                @endif
                                                <p class="smaller text-danger">
                                                    {{ $errors->first('customer_postal_code') }}</p>
                                            </fieldset>
                                        </div>
                                            ...
                                            <fieldset>
                                                ...
                                                @else
                                                    <input type="text" name="customer_address" id="customer_address"
                                                        ...
                                                        required />
                                                @endif
                                                <p class="smaller text-danger">{{ $errors->first('customer_address') }}
                                                </p>
                                            </fieldset>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-lg-4 col-md-6 col-12">
                                            <div class="contact-item">
                                                <div class="contact-icon">
                                                    <input type="radio" name="courier" class="form-check-input"
                                                        value="jne">
                                                </div>
                            ...
                            <div class="shipping-address-area billing-area">
                                <div class="minicart-btn-area d-flex align-items-center justify-content-between flex-wrap">
                                    <a href="{{ route('front.list_cart') }}"
                                        class="checkout-page-btn minicart-btn btn-secondary">KEMBALI KE KERANJANG</a>
                                    @if (auth()->guard('customer')->check())
                                        <button type="submit"
                                            class="checkout-page-btn minicart-btn btn-primary">LANJUTKAN KE
                                            PEMBAYARAN</button>
                                    @else
                                        <a href="{{ route('customer.login') }}"
                                            class="checkout-page-btn minicart-btn btn-primary">LANJUTKAN KE PEMBAYARAN</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                                ...
                                <h3 class="d-none d-lg-block mb-0 text-center heading_24 mb-4">Ringkasan pesanan</h4>
                                    @foreach ($carts as $data)
                                        <div class="minicart-item d-flex">
                                            <div class="mini-img-wrapper">
                                                <img class="mini-img" src="{{ Storage::url($data['product_image']) }}"
                                                    alt="img">
                                            </div>
                                            <div class="product-info">
                                                <h2 class="product-title">
                                                    @if (!empty($data['product_color']))
                                                        <a href="{{ $data['product_slug'] }}">{{ $data['product_name'] }}
                                                            {{ $data['product_color'] }}</a>
                                                    @endif
                                                    @if (!empty($data['product_nicotine']))
                                                        <a href="{{ $data['product_slug'] }}">{{ $data['product_name'] }}
                                                            {{ $data['product_nicotine'] }}</a>
                                                    @endif
                                                </h2>
                                                <p class="product-vendor">IDR {{ number_format($data['product_price']) }}x
                                                    {{ $data['qty'] }}</p>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="cart-total-box mt-4 bg-transparent p-0">
                                        ...
                                        <div class="subtotal-item discount-box">
                                            ...
                                            <p class="subtotal-value" id="total">IDR
                                                {{ number_format($total_amount) }}</p>
                                        </div>
                                    </div>
                            </div>
                        </div>
